...
class PasswordResetsRepository extends Repository {
  public function model() {
    return 'App\Models\Chombrain\PasswordResets';
  }
  ...
  public function create(array $data) {
    ...
    $row = new PasswordResets;

    DB::transaction(function () use ($data, $row) {
      if (!PasswordResets::canCreate($data)) {
        throw new \Exception("Permission denied");
      }

      PasswordResets::beforeCreate($data);
  ...
  public function list(array $args = []) {
    PasswordResets::beforeList();

    $output = PasswordResets::filterRecordRules( PasswordResets::query() );
      ...
      $output = $output->where(function ($query) use ($simpleSearch) {
        $query
          ->orWhere('id','LIKE','%'.escapeLike($simpleSearch).'%')
            ...
            ->orWhere('token','LIKE','%'.escapeLike($simpleSearch).'%')
        ;
      });
    }
    ...
    PasswordResets::afterList($output);
