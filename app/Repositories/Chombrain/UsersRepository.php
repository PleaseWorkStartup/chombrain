...
class UsersRepository extends Repository {
  public function model() {
    return 'App\Models\Chombrain\Users';
  }
  ...
  public function create(array $data) {
    ...
    DB::transaction(function () use ($data, $row) {
      if (!Users::canCreate($data)) {
        throw new \Exception("Permission denied");
      }

      Users::beforeCreate($data);
  ...
  public function list(array $args = []) {
    ...
    $output = Users::filterRecordRules( Users::query() );
      ...
      $output = $output->where(function ($query) use ($simpleSearch) {
        $query
          ->orWhere('id','LIKE','%'.escapeLike($simpleSearch).'%')
            ->orWhere('name','LIKE','%'.escapeLike($simpleSearch).'%')
            ->orWhere('email','LIKE','%'.escapeLike($simpleSearch).'%')
            ->orWhere('email_verified_at','LIKE','%'.escapeLike($simpleSearch).'%')
            
            
            
            ->orWhere('profile','LIKE','%'.escapeLike($simpleSearch).'%')
            ->orWhere('settings','LIKE','%'.escapeLike($simpleSearch).'%')
